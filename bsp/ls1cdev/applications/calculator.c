...
struct rtgui_win * win1_ui_init(struct rtgui_win *parent)
{
	...
	rtgui_rect_t rect={30,30,350,270};
        struct rtgui_label *label = RT_NULL;
	...
	*button_num7,*button_prec,*button_point,*button_num0;
	...
	/* win1 Create */
	{
		win1=rtgui_win_create(parent,"                        Simple calculator -RTGUI",&rect,RTGUI_WIN_STYLE_CLOSEBOX);
		rtgui_object_set_event_handler(RTGUI_OBJECT(win1), win1_event_handler);
		RTGUI_WIDGET_BACKGROUND(win1) = RTGUI_RGB(0, 192, 192);
	}
        
	label = rtgui_label_create(" Simple calculator:Using RTGUI!");
	rtgui_widget_set_minwidth(RTGUI_WIDGET(label), 250);
	...
	/* button_clr Create */
	{
		button_clr=rtgui_button_create("clear");
		RTGUI_RECT(rect,220,180,40,40);
...
static void calculator_thread_entry(void* parameter)
{
    struct rtgui_app* application;
    struct rtgui_win* win;

    /* create calculator GUI application */
    application = rtgui_app_create("calculator");
    RT_ASSERT(application != RT_NULL);
...
int calculator(void)
{
    ...
    tid = rt_thread_create("calculator", calculator_thread_entry, RT_NULL, 
        2048, 20, 20);
    if (tid != RT_NULL)
